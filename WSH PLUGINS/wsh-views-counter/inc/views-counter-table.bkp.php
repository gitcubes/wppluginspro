...
class WSH_Views_Counter_List_Table extends WP_List_Table {
	...
	public function extra_tablenav( $which ) {
		if ( 'top' === $which ) {
			...
			<form action="" method="get" id="items-search">
				...
				?>
				<select name="cvc_period" id="search-item-id-period-select">
				...
				?>
				<select name="cvc_post_type" id="search-item-id-post-type-select">
	...
	public function get_columns() {
		...
		if ( array_key_exists( $cvc_report, $cvc_reports ) && 1 === $cvc_report ) {
			...
			unset( $columns['post_id'], $columns['post_title'], $columns['author'], $columns['published_posts'] );
		} elseif ( 2 !== $cvc_report ) {
			unset( $columns['published_posts'] );
		}
