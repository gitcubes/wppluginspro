...
jQuery(document).ready(function ($) {
    ...
    if (
        ...
        $(sc_ad).hide();
        $(sc_cu).show();
        $(".seopress-schema-mapping-label").hide();
    }
    ...
    $("#seopress-your-schema input[name='seopress_pro_rich_snippets_type']").on('change', function() {
        ...
        if (seopress_rs_type == "custom") {
            ...
            $(".seopress-schema-mapping-label").show();
        }
    });
    ...
    /**
     * Custom Schema Script Tag Validation
     * Validates that custom schema textarea contains required <script> tags
     */
    // Selector for custom schema textareas (covers both manual and automatic schema contexts)
    var customSchemaSelector = 'textarea[name*="seopress_pro_rich_snippets_custom"], textarea.seopress-custom-schema-textarea, textarea.manual_custom_global';

    function validateCustomSchemaScriptTags(textarea) {
        var value = textarea.val().trim();
        var validationNotice = textarea.closest('.wrap-rich-snippets-item, .wrap-rich-snippets-custom')
            .find('.seopress-custom-schema-validation');

        // If empty, don't show error (let required validation handle it if needed)
        if (!value) {
            validationNotice.hide();
            return true;
        }

        // Check for opening script tag with type="application/ld+json" and closing tag
        var valueLower = value.toLowerCase();
        var hasOpeningTag = valueLower.includes('<script');
        var hasClosingTag = valueLower.includes('</script>');
        var hasJsonLdType = valueLower.includes('type="application/ld+json"') || valueLower.includes("type='application/ld+json'");

        if (!hasOpeningTag || !hasClosingTag || !hasJsonLdType) {
            validationNotice.slideDown(200);
            return false;
        } else {
            validationNotice.slideUp(200);
            return true;
        }
    }

    // Validate on blur (when user leaves the field)
    $(document).on('blur', customSchemaSelector, function() {
        validateCustomSchemaScriptTags($(this));
    });

    // Validate on input (real-time as user types)
    $(document).on('input', customSchemaSelector, function() {
        var textarea = $(this);
        // Debounce to avoid too frequent validation
        clearTimeout(textarea.data('validation-timeout'));
        textarea.data('validation-timeout', setTimeout(function() {
            validateCustomSchemaScriptTags(textarea);
        }, 500));
    });

    // Validate all custom schema fields on page load (for existing content)
    $(customSchemaSelector).each(function() {
        var textarea = $(this);
    ...
    $(document).on('submit', 'form#post, form#edittag', function(e) {
        var hasErrors = false;

        $(customSchemaSelector).each(function() {
            if (!validateCustomSchemaScriptTags($(this))) {
                hasErrors = true;
            }
        });
        ...
        if (hasErrors) {
            ...
            var firstError = $('.seopress-custom-schema-validation:visible').first();
