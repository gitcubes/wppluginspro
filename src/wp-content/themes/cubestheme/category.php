...
$blog_hero_title = single_cat_title('', false);
$blog_hero_description = category_description();
...
<section class="hero-section">
    <div class="container">
        <div class="lead-content-holder animation" data-animation="slideUp" data-delay="0.15s">
            <span class="label"><?php printf(esc_html__('Category', 'cubestheme')); ?></span>
...
<section class="all-blog-posts">
    <div class="container">
            ...
            <div class="categories-tab-nav border pill-tabs">
                ...
                <?php if ($blog_categories) : ?>
                    <?php foreach ($blog_categories as $category) : ?>
                        <?php
                        if ((int) $category->term_id === $default_category_id || $category->slug === 'uncategorized') {
                            continue;
                        }
                        ?>
                        <a href="<?php echo esc_url(get_category_link($category->term_id)); ?>"
                            class="categories-tab <?php echo ((int) $current_category->term_id === (int) $category->term_id) ? 'is-active' : ''; ?>">
        ...
        <?php
        $pagination_base = trailingslashit(get_category_link($current_category->term_id)) . user_trailingslashit('page/%#%/', 'paged');
        ?>
