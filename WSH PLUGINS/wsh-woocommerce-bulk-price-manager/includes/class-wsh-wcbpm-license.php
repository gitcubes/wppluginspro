 ...
 */
class WSH_WCBPM_License {

	const OPTION_KEY        = 'wsh_wcbpm_license_key';
	const OPTION_STATUS     = 'wsh_wcbpm_license_status';
	const OPTION_EXPIRES    = 'wsh_wcbpm_license_expires';
	const OPTION_LAST_ERROR = 'wsh_wcbpm_license_last_error';

	const TRANSIENT_LAST_VERIFY = 'wsh_wcbpm_last_verify';
	...
	/**
	 * Always send site_url in a consistent format for signature/payload.
	 * Use home_url() without trailing slash differences.
	 */
	protected static function get_site_url_for_api() : string {
		// Keep consistent format (no trailing slash).
		return untrailingslashit( home_url() );
	}

	/**
	 * Build HMAC auth params required by license server permission_check().
	 *
	...
	protected static function build_auth_params( string $license_key, string $site_url, string $plugin_slug ) : array {
		...
		// If secret missing, we still return empty; server will reject with 401.
		if ( $secret === '' ) {
			return array();
		}
	...
	protected static function call_remote( string $action, array $extra = array() ) {
		...
		$body = array_merge(
			array(
				'action'         => $action,
				'site_url'       => home_url(),
				'plugin_slug'    => defined('WSH_WCBPM_PLUGIN_SLUG') ? WSH_WCBPM_PLUGIN_SLUG : 'wsh-woocommerce-bulk-price-manager',
				'plugin_version' => defined( 'WSH_WCBPM_VERSION' ) ? WSH_WCBPM_VERSION : '1.0.0',
			),
			$extra
		);
		*/
		...
		// Add HMAC auth params (requires license_key to exist in $body)
	...
	public static function activate( string $key ) : array {
		...
		if ( '' === $key ) {
			...
			return array( 'success' => false, 'message' => __( 'Please enter a license key.', 'wsh-wcbpm' ) );
		}

		$response = self::call_remote( 'activate', array( 'license_key' => $key ) );
		$result   = self::handle_remote_response( $response, $key );

		// Refresh verify cache on manual activation attempt.
	...
	public static function deactivate() : array {
		...
		if ( '' === $key ) {
			...
			return array( 'success' => true, 'message' => __( 'License cleared locally.', 'wsh-wcbpm' ) );
		}
	...
	public static function verify( bool $force = false ) : array {
		...
		if ( '' === $key ) {
			self::store_license_data( '', '', '', '' );
			return array( 'success' => false, 'message' => __( 'No license key saved.', 'wsh-wcbpm' ) );
		}
		...
		if ( ! $force ) {
			...
			if ( $last && ( time() - (int) $last < DAY_IN_SECONDS ) ) {
				return array(
					...
					'message' => __( 'License status is cached.', 'wsh-wcbpm' ),
				);
			}
		}
